...
import { MarkerSwipe } from '@/components/ccm/marker-swipe'
...
type ButtonProps = {
   ...
   * it reads on bone. `dark` flips the text to white so it stays legible over
   * the acid marker on a `bg-neutral-950` panel (the marker itself stays acid
   * either way). For `solid` it only drives the focus-ring offset color.
   */
  tone?: Tone
  withArrow?: boolean
} & (
 ...
 *   the swipe on hover.
 * - `solid` is a conventional rectangular acid button with rounded corners
 *   and a clear button affordance. The locked treatment for FORM submit
 *   buttons (footer, contact block, /contact), where the marker-swiped text
 *   reads as ambiguous next to an input. Marker swipe stays reserved for
 *   hero/nav/link CTAs.
 *
