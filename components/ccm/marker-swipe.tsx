...
/**
 * Acid highlighter swipe, CCM Greg Isenberg "Blog" style.
 *
 ...
 *
 * The fill is a literal acid hex (`#cfe34d`, sRGB fallback for the acid OKLCH
 * primary) set via the inline CSS `fill` property (with a matching presentation
 * attribute as a fallback) rather than `fill="var(...)"`. Safari and iOS do not
 * evaluate `var()` inside an SVG presentation attribute, so the swipe failed to
 * paint there. A literal CSS `fill` value paints solid acid in every engine.
 */
const MARKER_ACID = '#cfe34d'
